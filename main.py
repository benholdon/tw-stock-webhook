...
# In-memory queues are retained only for the Taiwan stock command/order path.
...
def reply_line_message(reply_token: str, text: str) -> bool:
    ...
    try:
        ...
        logging.error("Reply error: %s", exc)
        return False
...
def line_event_id(ev: dict, user_id: str, payload_key: str, *, message_id: str = "") -> str:
    webhook_event_id = str(ev.get("webhookEventId") or "").strip()
    if webhook_event_id:
        return f"line:{webhook_event_id}"
    if message_id:
        return f"line-message:{message_id}"
    seed = f"{user_id}|{payload_key}|{ev.get('timestamp', '')}|{ev.get('replyToken', '')}"
    return "line-fallback:" + hashlib.sha256(seed.encode("utf-8")).hexdigest()[:40]


def store_approval_decision(ev: dict, *, user_id: str, text: str, payload_key: str, message_id: str = "") -> dict:
    event_id = line_event_id(ev, user_id, payload_key, message_id=message_id)
...
async def handle_webhook(request: Request, x_line_signature: Optional[str] = Header(None)):
    ...
    if x_line_signature and LINE_CHANNEL_SECRET and not verify_signature(body_bytes, x_line_signature):
    ...
    for ev in data.get("events", []):
        ...
        if ev_type == "postback":
            ...
            logging.info("Received Postback data: %s", pb_data)
            params = dict(p.split("=", 1) for p in pb_data.split("&") if "=" in p)
            action = params.get("action", "")

            # AI Orchestration human approval buttons are kept completely separate
            # from the Taiwan stock trading postback queue.
            if action == "AI_APPROVAL":
                approval_id = params.get("approval_id", "").strip()
                decision = params.get("decision", "").strip().lower()
                if decision not in {"ok", "no"} or not approval_id:
                    reply_line_message(reply_token, "審核按鈕資料無效，未執行任何動作。")
                    continue
                decision_text = f"{decision} {approval_id}"
                try:
                    ...
                    logging.exception("Failed to persist approval postback: %s", exc)
                    raise HTTPException(status_code=503, detail="approval persistence unavailable")
                reply_line_message(reply_token, "已收到審核決定並安全保存，等待本機 Approval Gate 確認。")
                continue

            # Existing Taiwan stock order postback path.
            ...
            logging.info("Received text command: %s", user_text)
            ...
            if is_approval_reply:
                try:
                    store_approval_decision(
                        ...
                        user_id=user_id,
                        text=user_text,
                        payload_key=user_text,
                        message_id=str(msg.get("id") or ""),
                    )
                except Exception as exc:
                    logging.exception("Failed to persist approval text reply: %s", exc)
                    raise HTTPException(status_code=503, detail="approval persistence unavailable")
